...
namespace B23_Ex02_Ariel_315363366_Adi_206820045
{
    ...
    class Game
    {
        ...
        {
            return this.m_Grid.AmountOfAvialibleCell;
        }

        public int GetGridSize()
        {
            return this.m_Grid.GetGridSize();
        }

        public bool IsCellEmpty(int i_X, int i_Y)
        {
            return this.m_Grid.IsCellEmpty(i_X, i_Y);
        }
        ...
        {
            return this.m_Grid.IsNextMoveValid(i_X, i_Y);
        }
        ...
        private bool isSequenceInRow(eMarks i_PlayerMark)
        {
            ...
            for (int x = 0; x < gridSize; x++)
            {
                ...
                {
                    eMarks currenctCellContent = this.m_Grid.GetCellContent(x, y);
        ...
        private bool isSequenceInCol(eMarks i_PlayerMark)
        {
            ...
            for (int x = 0; x < gridSize; x++)
            {
                ...
                for (int y = 0; y < gridSize; y++)
                {
                    eMarks currenctCellContent = this.m_Grid.GetCellContent(y, x);
        ...
        private bool isSequenceInLeftDiagonal(eMarks i_PlayerMark) {
            ...
            for (int x = 0; x < gridSize; x++)
            {
                eMarks currenctCellContent = this.m_Grid.GetCellContent(x, x);
        ...
        private bool isSequenceInRightDiagonal(eMarks i_PlayerMark)
        {
            ...
            {
                eMarks currenctCellContent = this.m_Grid.GetCellContent(gridSize - 1 - x, x);
