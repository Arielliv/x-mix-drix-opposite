...
namespace B23_Ex02_Ariel_315363366_Adi_206820045
{
    class GameController
    {
        ...
        private void setPlayers(eGameModes i_GameMode) {
            this.m_Players[0] = new Player(ePlayerTypes.Person, eMarks.X);
            if (i_GameMode == eGameModes.TwoPlayers)
            {
                this.m_Players[1] = new Player(ePlayerTypes.Person, eMarks.O);
            } 
            else
            {
                this.m_Players[1] = new Player(ePlayerTypes.Computer, eMarks.O);
            }
        }
